...
function parafinResponse2(
  mergedResultAsync: ResultAsync<
    [
      ResultAsync<PartnerResponse, ParafinError>,
      ResultAsync<BusinessCoreResponse, ParafinError>,
      ResultAsync<OfferCollectionResponse, ParafinError>,
  ...
  const loo = mergedResultAsync.then((res) => {
    if (res.isOk()) {
      res.value[0].then((resPartner) => {
        if (resPartner.isOk()) {
          response.partnerName = resPartner.value.partnerName
          response.partnerSlug = resPartner.value.partnerSlug
        }
      })

      res.value[1].then((resBusiness) => {
        if (resBusiness.isOk()) {
          response.externalId = resBusiness.value.externalId
        }
      })

      res.value[2].then((resOffer) => {
        if (resOffer.isOk()) {
          response.approvalAmount = resOffer.value.approvalAmount
        }
      })

      res.value[3].then((resAdvance) => {
        if (resAdvance.isOk()) {
          response.acceptedAmount = resAdvance.value.acceptedAmount
          response.outstandingAmount = resAdvance.value.outstandingAmount
          response.paidAmount = resAdvance.value.paidAmount
          response.estimatedPayoffDate = resAdvance.value.estimatedPayoffDate
          response.verified = resAdvance.value.verified
          response.totalAdvances = resAdvance.value.totalAdvances
        }
      })

      res.value[4].then((resOptIn) => {
        if (resOptIn.isOk()) {
          response.opted = resOptIn.value.opted
        }
      })
    }
  })
